...
```{marginfigure}
Para ser más exactos (y hacernos les capes), modelamos la probabilidad con la siguiente fórmula:
$$P(y=1 | X) = \frac{1}{1 + e^{-(\beta_{0} + \beta_{1}X_{1} + \beta_{2}X_{2} + \beta_{3}X_{3} + \beta_{4} X_{4})}}$$
...
En este caso particular, la aplicación de los p-valores en el sentido puramente estadístico del término puede ser confusa: dado que se trata de herramientas de la estadística inferencial y aquí estamos lidiando con una población (la totalidad de les diputades) pareciera que aquí no corre la posibilidad de hacer inferencias.

No obstante, uno podría argumentar dos cosas:

- Que se puede usar el p-valor de forma "conceptual" y no "estadística" (gracias a [Diego Koslowski](https://twitter.com/Diego_Koz) y [Octavio Bramajo](https://twitter.com/octaviobramajo) por sus comentarios al respecto). Es decir, lo usamos como una forma de ver qué variables tienen mayor influencia sobre la votación. También podríamos haber visto los tamaños de los coeficientes y ya.

- Que (y aquí la cosa se torna un poco más filosófica) la votación que observamos es una realización de un proceso aleatorio. Es decir que si realizáramos la votación muchas veces, obtendríamos un resultado diferente. Y en ese sentido la votación que observamos se trataría de una "muestra", no de una población, sino de muchas otras votaciones posibles. Gary King en [este libro](https://www.cambridge.org/ar/academic/subjects/politics-international-relations/political-theory/unifying-political-methodology-likelihood-theory-statistical-inference?format=PB&isbn=9780521366977) lo explica de forma bastante clara.

Además de todo esto, hay una gran discusión en la comunidad estadística acerca de los p-valores y los problemas que genera su] uso. Les recomendamos como primera aproximación [esta charla](https://lacion.rbind.io/talk/2019_pvalor/) que dio [Laura Ación](https://lacion.rbind.io/) al respecto. [Acá está el video de la charla completa](https://www.youtube.com/watch?v=by-j2BdrPIo) (junto con las exposicones de Walter Sosa Escudero y Germán Feierherd).
...
$$P(y=1 | X) = \frac{1}{1 + e^{-(1 - 0.004 \times edad)}}$$
...
$$P(y=1 | X) = \frac{1}{1 + e^{-(1 - 0.004 \times 34)}}=0.703$$
